    ...
    <main>
      ...
      <article>
        <header style={{ padding: 'clamp(140px, 20vh, 200px) 8vw 48px', maxWidth: 800, margin: '0 auto' }}>
          ...
          <span style={{ display: 'block', marginTop: 24, fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--color-accent-light)' }}>{post.category}</span>
          ...
          <time style={{ fontSize: 13, color: 'rgba(245,240,232,0.5)' }}>{fmtDate(post.date)}</time>
        </header>

        <img src={post.image_url || '/assets/ph-arch-1.svg'} alt="" style={{ width: '100%', maxHeight: 520, objectFit: 'cover' }} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 24 }}>
              {related.map((r) => (
                <Link key={r.id} to={`/blog/${r.slug}`} data-cursor style={{ textDecoration: 'none', color: 'inherit' }}>
                  <img src={r.image_url || '/assets/ph-arch-1.svg'} alt="" style={{ width: '100%', height: 180, objectFit: 'cover', marginBottom: 12 }} />
